import { showData, formatDate } from "../../../assets/scripts/helper.js";

/**
 * Generates a billing table row HTML element for a client's billing information.
 *
 * @param {object} billing - The client's billing data.
 * @param {number} index - The index of the billing row in the table.
 * @returns {string} - The HTML markup for the billing table row.
 */
export function billingTableRow(billing, index) {
    ...
    const clientDisconnected =  connectionStatus !== null && 
                                ...
                                connectionStatus === window.connectionStatusTypes.Disconnected) ? 
                                true : false
        ...
        <div class="table-info account" data-client-index="${index}" data-client-id="${billing.id}" data-account-number="${billing.accountNumber}" data-meter-number="${billing.meterNumber}" data-full-name="${billing.fullName}">
            ...
            <div class="table-info__options" data-client-id="${billing.id}">
                ${
                    !clientDisconnected ? `
                        <p>Menu</p>
                        <div class="table-info__options-item-box billing" data-client-has-paid="${clientHasPaid}" data-client-id="${billing.id}" data-client-has-bills="${clientHasBills}">
                            ${
                                !clientDisconnected ? [newBillButton, payBillButton, printBillButton].join("") : ''
                            }
                        </div>    
                    ` : ''
                }
            ...
            <div class="table-info__item">
                <p>${showData(billing.accountNumber)}</p>
            </div>
            <div class="table-info__item">
                <p>${showData(billing.fullName)}</p>
            </div>
            <div class="table-info__item">
                <p>${showData(billing.meterNumber)}</p>
            </div>
            <div class="table-info__item">
