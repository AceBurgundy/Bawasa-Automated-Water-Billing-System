export function billingTableRow(billing, index) {
    ...
    const { firstReading, secondReading, consumption, billAmount, paymentExcess, penalty, dueDate, disconnectionDate, remainingBalance, paymentStatus, paymentAmount } = billData;
    ...
    const connectionStatus = billing.Client_Connection_Statuses.length > 0 ? billing.Client_Connection_Statuses[0] : {}
    const clientDisconnected = connectionStatus !== {} && connectionStatus === window.connectionStatusOptions.Disconnected
    
    const reconnectButton = clientDisconnected ?
